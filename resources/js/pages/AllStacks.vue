    ...
    <div class="d-flex flex-column align-items-center gap-5">
        ...
        <div v-if="display_confirm_email_sent" class="position-relative">
            ...
            <div class="alert alert-warning alert-dismissible fade show d-flex justify-content-between align-items-center" role="alert">
                <span class="alert-text">Confirmation email sent to {{ user.email }}</span>
            </div>
    
    
        </div>
        
        <div v-if="loading_stacks" class="spinner-border" role="status">
        </div>
...
export default {
    ...
    methods: {
        ...
        async sendEmailVerificationLink(){
            try {
                let res = await window.axios.post('/email/verification-notification');
                if(res.status === 200) {
                    this.user_store.hideConfirmEmail();
