<template>
    ...
    <div class="study-wrapper">
                ...
                <div class="notecard-inner">
                    <transition>
                        <div
                            :class="{
                                'position-absolute':
                                    !show_front || in_transition,
                            }"
                            :key="`${current_card_index}-front`"
                            v-if="show_front"
                            class="notecard-content hide-toolbar w-100"
                        >
                            <quill-editor
                                :syntax-highlight="customSyntaxHighlight"
                                v-model:content="current_card.front"
                                :readOnly="true"
                                contentType="html"
                                theme="snow"
                            />
                        ...
                        <div
                            ...
                            class="w-100 notecard-content hide-toolbar"
                        >
                            <quill-editor
                                :syntax-highlight="customSyntaxHighlight"
                                :key="`${current_card_index}-back`"
                                v-model:content="current_card.back"
                                :readOnly="true"
                                contentType="html"
                                theme="snow"
                            />
        ...
        <span :class="{ invisible: !show_message }" class="mb-3"
            >Tap or click the notecard to see the
        ...
        >
            <div @click="loading = true">
                <study-buttons
                    ...
                    "
                    :notecard_id="
                        this.notecards[current_card_index]?.notecard_id
                    "
                ></study-buttons>
            </div>
            <span
                @click="displayDefs"
                class="align-self-start emulate-link fw-bold"
                >Definitions</span
            >
        </template>
...
export default {
    ...
    computed: {
        ...
        show_message() {
            return (
                (this.show_front && !this.has_seen_back) ||
